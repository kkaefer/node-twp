...
Protocol.prototype.parseMessage = function(message) {
    ...
    var message = this.messages[message.id].parse(message);
    var types = this.interface.types;
    message.extensions = message.extensions.map(function(extension) {
        if (!types[extension.id]) {
            throw new Error('Unknown extension ID "' + extension.id + '"');
        }
        return {
            name: types[extension.id].name,
            data: types[extension.id].parse(extension)
        };
    }, this);
    return message;
};
