...
Connection.prototype.read = function() {
    ...
    while (this.buffer.length > 0) {
        ...
        if (!this.message && (tag < 4 || tag > 12)) {
            throw new Error('Protocol error, expected message or registered extension');
        }
        ...
        if (tag === 0) { // End of Content
            ...
            if (!this.stack.length) {
                ...
                delete this.message;
                this.emit('message', message.name, message.data);
            }
        }
