...
Connection.authenticationErrors = {
    0: 'other error',
    1: 'bad certificate',
...
Connection.prototype.authenticateMessage = function(message) {
    var signature = message.extensions.filter(function(ext) { return ext.id === 28; });
    if (this.options.requireAuthentication || signature.length) {
        // We either require a signature or there's a signature attached to this message.
        if (!signature.length) {
            var error = 'Message is not signed';
        } else if (!this.certificate) {
            var error = 'Server did not send certificate';
        } else {
            var mac = signature[0].fields[0];
            var fields = this.messageBuffer.slice(this.messageBuffer[0] == 12 ? 5 : 1, signature[0].offset);
            var valid = Signature.verify(fields, mac, this.certificate);
            var error = valid ? null : 'Signature is invalid';
        }
...
Connection.prototype.verifyMessage = function() {
    try {
        if (this.message.id === 27) {
            // We received a certificate.
            var message = this.protocol.parseMessage(Authentication.messages.Certificate, this.message);
            this.certificate = Signature.DERtoCER(message.data.data);
        } else if (this.message.id === 29) {
            // Authentication error.
            var message = this.protocol.parseMessage(Authentication.messages.AuthenticationError, this.message);
            var description = Connection.authenticationErrors[message.data.code];
            this.emit('authenticationError', message.data.code, description, message.data.detail);
        } else {
            // Regular message.
            var message = this.protocol.parseMessage(this.message);
            var err = this.authenticateMessage(message);
            if (err) {
                this.emit('untrustedMessage', err, message.name, message.data, message.extensions);
            } else {
                this.emit('message', message.name, message.data, message.extensions);
            }
        }
    } catch(err) {
